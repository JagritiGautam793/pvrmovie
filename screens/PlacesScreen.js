import { Pressable, StyleSheet, Text, View, TextInput, ImageBackground, FlatList } from 'react-native';
...
const PlacesScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setCities(result);
        console.log('Fetched cities:', result); // Log the fetched data directly here
      } catch (error) {
  ...
  console.log('Rendering PlacesScreen...');
  console.log('Current cities state:', cities);

  return (
    <View>
      <View style={{ margin: 10, padding: 10, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderColor: "#E0E0E0", borderWidth: 2, borderRadius: 30 }}>
      ...
      <FlatList
        ...
        data={cities}
        renderItem={({ item }) => (
          <Pressable onPress={() => selectCity(item.city,item._id)} style={{ marginVertical: 0, marginHorizontal: 20 }}>
            <ImageBackground imageStyle={{ borderRadius: 8 }}
