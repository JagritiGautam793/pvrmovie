...
import RazorpayCheckout from "react-native-razorpay";import { client } from "../pvr/sanity";

const ConfirmationScreen = () => {
  const route = useRoute();
  const navigation=useNavigation();
  console.log("here are " ,route.params);  
  ...
  const total = cart
    ...
    // this peice of code will not allow to go back to prev screen
    ...
    useEffect(()=>{ 
      const backAction=()=>{
        const createTwoButtonAlert = () =>
          Alert.alert('Want to end Session', 'Go Back to Msin Screen', [
            {
              text: 'Cancel',
    ...
    <View style={{ padding: 20 }}>
      ...
      >
        ...
        <View style={{marginTop:8}}>
                ...
                <Text style={{fontSize:15,fontWeight:"500",marginTop:4,color:"gray"}}>AUDI 02. CLASSIC</Text>  
                <Text style={{color:"red",marginTop:4,fontWeight:"500"}}>{route.params.seats}|{route.params.showtime}</Text>
                {/* mall no */} 
            ...
            <View style={{marginTop:10,flexDirection:"row",alignItems:"center",justifyContent:"space-between"}}>
                ...
                <Text style={{fontWeight:"bold",fontSize:17}}>₹{grandTotal}</Text>
            </View>  

            <View style={{marginTop:10,flexDirection:"row",alignItems:"center",justifyContent:"space-between"}}>
                <Text>TICKETS {route.params.selectedSeats.length}</Text> 
                <Text style={{fontSize:15,fontWeight:"500"}}>₹{route.params.selectedSeats.length*220}</Text>
            </View> 
            ...
            <View style={{marginTop:10,flexDirection:"row",alignItems:"center",justifyContent:"space-between"}}>
                <Text style={{fontSize:15,fontWeight:"500"}}>Convenience Fee</Text> 
                <Text style={{fontSize:15,fontWeight:"500"}}>₹87</Text>
            </View>
        </View>

        <Pressable onPress={pay} style={{marginTop:10,backgroundColor:"#FFC72C",padding:10,borderRadius:4,justifyContent:"center",alignItems:"center"}}>
